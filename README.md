...
> `OPENAI_API_KEY` (и при необходимости `OPENAI_BASE_URL` для локального
> vLLM/Ollama). **Без ключа** проект работает в extractive-режиме — ответ
> собирается из найденных фрагментов. Запуск на чистой машине ключа не требует.
...
| Вопрос | Ожидание |
|--------|----------|
| **В каком году истекает договор аренды Байконура?** | ответ по фрагментам про Байконур + источник (doc_id, score) |
...
<!-- LOGS -->
Вывод `uv run python scripts/check_generator.py` (extractive-режим, без LLM-ключа):

```text
LLM включена: False (нет ключа -> extractive demo-режим)

--- Контекст есть: «В каком году истекает договор аренды Байконура?» ---
режим: extractive, отказ: False
Источники:
  [1] doc_id=146, score=0.577 (rerank), «В целях совершенствования договорно-правовой базы … космодрома Байконур …»
  [2] doc_id=2,   score=0.086 (rerank), «Город Байконур и космодром Байконур …»

--- Контекст есть: «чем представлены органические остатки?» ---
режим: extractive, отказ: False
Источники:
  [1] doc_id=0,    score=0.935 (rerank), «В протерозойских отложениях органические остатки … известковыми выделениями … водорослей …»
  [2] doc_id=1535, score=0.105 (rerank), «Углеводы — весьма обширный класс органических соединений …»

--- Контекст есть: «из чего состоят белки?» ---
режим: extractive, отказ: False
Источники:
  [1] doc_id=1575, score=0.864 (rerank), «Белки́ (протеи́ны) — высокомолекулярные органические вещества из альфа-аминокислот …»

--- Negative: «как приготовить борщ и какие нужны ингредиенты?» ---
режим: refused, отказ: True
Ответ: В базе не найдено релевантных фрагментов по этому вопросу. Ответить по имеющимся данным невозможно.
  [1] doc_id=889, score=0.005 (rerank)   ← все кандидаты ниже порога 0.1 → отказ
  [2] doc_id=138, score=0.003 (rerank)
```
